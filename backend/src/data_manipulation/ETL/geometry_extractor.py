...
def extract_geometry_values(folder_path):
    ...
    if not folder_path:
        print(f"Error: Folder not found: {sys.argv[1] if len(sys.argv) > 1 else folder_path}", flush=True)
        return None

    results_path = os.path.join(folder_path, "Results.xml")
    if not os.path.exists(results_path):
        print(f"Error: Results.xml not found in {folder_path}", flush=True)
        return None

    if not is_geometry_folder(folder_path):
        print("Error: Not a geometry check folder", flush=True)
        return None
...
if __name__ == "__main__":
    ...
    # Force unbuffered stdout (fixes no output in Cursor/VS Code terminal)
    try:
    ...
    if len(sys.argv) < 2:
        print("Usage: geometry_extractor.py <folder_path>", flush=True)
        sys.exit(1)
    ...
    if data is None:
        print("Failed to extract geometry values", flush=True)
        sys.exit(1)
    ...
    for key, label, leaf_name in [
        ...
        leaves = data[key]
        if leaves:
            print(f"\n{label}: {len(leaves)} leaves", flush=True)
            for idx, val in leaves.items():
                print(f"  {leaf_name}{idx} [mm] = {val}", flush=True)
        else:
            print(f"\n{label}: No leaves found", flush=True)